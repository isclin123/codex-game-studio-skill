---
name: game-studio
description: Build, modify, debug, and polish games and interactive prototypes across browser games, Godot, Unity, and Unreal Engine. Use when Codex is asked to create a game, game jam prototype, arcade game, puzzle game, 2D/3D game, Phaser/Three.js/canvas game, Godot project, Unity project, Unreal project, gameplay mechanic, level, HUD, sprites, assets, QA pass, playtest plan, or to coordinate a virtual AI game-studio team.
---
...
7. **Run or validate the game.** Start the dev server, Godot editor/headless run, Unity tests, or Unreal automation when available. If an engine is not installed, make code/project changes and provide exact run steps.
